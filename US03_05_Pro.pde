...
void draw()
{
  ...
  if(zeilen>0)  //Wenn Zeilenanzahl größer 0 (Einträge in Tabelle), dann...
  {
    for(int i=0;i<zeilen;i++)  //gehe jede Zeile von Beginn an durch
    {
      ...
      //{
        text(tabelle.getString(i,"Datum/Uhrzeit"),10,20*i+425);  //schreibe den Eintrag der Spalte Datum/Uhrzeit auf das Canvas
        text(tabelle.getString(i,"Ereignis"),250,20*i+425);  //schreibe den Eintrag der Spalte Ereignis auf das Canvas
        text(tabelle.getString(i,"Benutzer"),400,20*i+425);  //schreibe den Eintrag der Spalte Benutzer auf das Canvas
      //}      
    }
  }  
}
