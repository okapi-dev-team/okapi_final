...
const int DHTPIN=A0; //Arduino Pin für DHT Sensor
const int BODENFEUCHTIGKEIT_SENSOR=A1;  //Arduino Pin für Bodenfeuchtigkeitssensor
const int maxLuftfeuchtigkeit=60; //Maximale Luftfeuchtigkeit auf 60% festlegt
const int maxTemperatur=25; //Maximale Temperatur auf 25°C festlegt
const int minBodenfeuchtigkeit=30;  //Minimale Bodenfeuchtigkeit auf 30% festlegt
const int BODEN_LED=4;  //Arduino Pin für Signal LED (Bodenfeuchtigkeit)

#define DHTTYPE DHT22 //Festlegung, dass es sich um den Sensor DHT 22 (AM2302) handelt
...
DHT dht(DHTPIN, DHTTYPE); //Objekt für DHT Initalisierung mit Bezeichnung "dht", DHT Pin und DHT Type
...
{
  temperatur=dht.readTemperature(); //Auslesen der Temperatur aus dem Sensor und Speicherung unter der Variable "temperatur"
  
  if(isnan(temperatur)) //Kontrolle des Messwert auf Sinnhaftigkeit (NaN = not a number). Wenn keine Zahl als Messwert vorliegt, ...
  {
...
{
  luftfeuchtigkeit=dht.readHumidity(); //Auslesen der Luftfeuchtigkeit aus dem Sensor und Speicherung unter der Variable "luftfeuchtigkeit"
  
  if(isnan(luftfeuchtigkeit)) //Kontrolle des Messwert auf Sinnhaftigkeit (NaN = not a number). Wenn keine Zahl als Messwert vorliegt, ...
  {
    return 0; //Rückgabe Wert 0
  }
  else
  {
    if(luftfeuchtigkeit>100)
    {
...
int messenBodenfeuchtigkeit()
{
  ...
  bodenfeuchtigkeit==map(messBodenfeuchtigkeit,0,220,0,100); //Umwandeln des Messwerts der Bodenfeuchtigkeit auf eine Skala von 0-100% und Speicherung unter der Variable "bodenfeuchtigkeit" (0% = trocken, 100% = feucht)
...
{  
  ...
  datenSenden[8]=9;  //Übertragen der gemessenen Bodenfeuchtigkeit
