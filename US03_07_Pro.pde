...
int[] seriellerEingang=new int[6];  //Definition eines int-Arrays seriellerEingang
...
void draw()
{
  ...
  if(zeilen>0)  //Wenn Zeilenanzahl größer 0 (Einträge in Tabelle), dann...
  {
    for(int i=0;i<zeilen;i++)  //gehe jede Zeile von Beginn an durch
    {
      ...
      //{
        text(tabelle.getString(i,"Datum/Uhrzeit"),10,20*i+410);  //schreibe den Eintrag der Spalte Datum/Uhrzeit auf das Canvas
        text(tabelle.getString(i,"Ereignis"),250,20*i+410);  //schreibe den Eintrag der Spalte Ereignis auf das Canvas
        text(tabelle.getString(i,"Benutzer"),400,20*i+410);  //schreibe den Eintrag der Spalte Benutzer auf das Canvas
      //}      
    }
  }  
}
...
void serialEvent(Serial seriellerPort)
{
  ...
  else  //Sonst (seriellerKontakt==true) = Kontakt bereits hergestellt
  {
    ...
    if(empfangeneSerielleBytes>5)  //Wenn Zählvariable größer 5 (sechs Einträge in Array), dann...
    {
