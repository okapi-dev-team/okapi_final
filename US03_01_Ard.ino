...
const int LED_PIN_BLAU=6; //Arduio Pin für LED 2 (blau)

int taster1Status=0;  //Tasterstatus Taster 1 (0=nicht gedrückt, 100=gedrückt)
int taster2Status=0;  //Tasterstatus Taster 2 (0=nicht gedrückt, 100=gedrückt)
...
void loop()
{
  if(Serial.available()>0)  //Wenn Daten am seriellen Port vorliegen, dann...
  {
    ...
    {
      digitalWrite(LED_PIN_BLAU,LOW); //Schalten des Arduino Pins (Spannung = LOW (0V)) für LED 2 (blau) -> LED leuchtet nicht
    }
    
    if(digitalRead(TASTER1_PIN)==HIGH)  //Wenn Taster 1 gedrückt, dann...
    {
      taster1Status=100;  //setze taster1Status=100
    }
    else  //Sonst...
    {
    ...
    {
      taster2Status=100;  //setze taster2Status=100
    }
    else  //Sonst...
    {
