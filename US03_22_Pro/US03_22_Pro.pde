...
PFont schriftart;  //Erstellen eines Objekts schriftart in Klasse PFont
PImage logo;  //Erstellen eines Objekts logo in Klasse PImage
...
int alarmanlageStatus,alarm,benutzer1,benutzer2,rechteBenutzer1,rechteBenutzer2,temperatur,luftfeuchtigkeit,bodenfeuchtigkeit,helligkeit;  //Definition der int-Variablen für empfangene Daten

boolean seriellerKontakt=false;  //Initalisierung der bool-Variable seriellerKontakt
boolean benutzer1Hold=false;  //Initalisierung von bool-Variablen als toggle (true=Benutzer ist angemeldet/hat master Rechte/GUI soll angezeigt werden/Schalterstellungen Jalousiesteuerung; false=Benutzer ist nicht angemeldet/hat slave Rechte/GUI soll nicht angezeigt werden)
boolean benutzer2Hold=false;
boolean masterRechteHold=false;
boolean gui=false;
boolean zeitschaltung=false;
boolean daemmerung=false;
...
void setup()
{
  ...
  schriftart=createFont("HelveticaNeue-Light",50,true);  //Initalisieren der Schriftart
  textFont(schriftart);  //Festlegen der verwendeten Schriftart
  logo=loadImage("okapi coding.png");  //Laden des Logos für Screensaver
...
void draw()
{
  ...
  if(gui)  //Wenn GUI angezeigt werden soll (Benutzer ist angemeldet)
  {
    ...
    if(mouseX>=500&&mouseX<=1000&&mouseY>=52&&mouseY<=98)  //Wenn Mauscursor über Schaltfläche für Abmeldung, dann...
    {
      fill(237,69,69);  //setze die Füllfarbe auf RGB Wert (237,69,69)
    }
    else  //Sonst...
    {
      noFill();  //keine Füllung der folgenden Formen
    }
    noStroke();  //kein Rahmen der folgenden Formen
    rect(503,53,497,45);
    ...
    if(mouseX>=590&&mouseX<=660&&mouseY>=205&&mouseY<=245)  //Wenn Mauscursor über Schaltfläche für Fenster auf, dann...
    {
      fill(150);  //setze die Füllfarbe auf Greyscale Wert 150
    }
    else  //Sonst...
    {
      noFill();  //keine Füllung der folgenden Formen
    ...
    if(mouseX>=590&&mouseX<=660&&mouseY>=255&&mouseY<=295)  //Wenn Mauscursor über Schaltfläche für Fenster zu, dann...
    {
      fill(150);  //setze die Füllfarbe auf Greyscale Wert 150
    }
    else  //Sonst...
    ...
    vertex(655,240);
    endShape();
    rect(590,205,70,40);  //zeichne Rechteck
    ...
    vertex(655,260);
    endShape();
    rect(590,255,70,40);  //zeichne Rechteck
    
    textSize(20);  //setze die Schriftgröße auf 20pt
    fill(0);  //setze die Schriftfarbe auf Greyscale Wert 0
    textAlign(CENTER,CENTER);  //Textausrichtung
    text("Fenster auf/zu",625,150);  //schreibe "Fenster auf/zu" auf das Canvas
    
    textSize(20);  //setze die Schriftgröße auf 20pt
    fill(0);  //setze die Schriftfarbe auf Greyscale Wert 0
    textAlign(CENTER,CENTER);  //Textausrichtung
    text("Jalousiesteuerung",875,150);  //schreibe "Fenster auf/zu" auf das Canvas        
    
    if(mouseX>=790&&mouseX<=960&&mouseY>=205&&mouseY<=245)  //Wenn Mauscursor über Schaltfläche für Zeitschaltung, dann...
    {
      fill(150);  //setze die Füllfarbe auf Greyscale Wert 150
    }
    else  //Sonst...
    ...
    if(zeitschaltung||((mousePressed==true&&mouseButton==LEFT)&&(mouseX>=790&&mouseX<=960&&mouseY>=205&&mouseY<=245)))  //Wenn toggle für Zeitschaltung=true oder in die Schaltfläche für Zeitschaltung geklickt wird, dann...
    {
      daemmerung=false;  //setze toggle für Dämmerungsschalter=false und für Zeitschaltung=true
      zeitschaltung=true;
      fill(62,250,128);  //setze die Füllfarbe auf RGB Wert (62,250,128)
    }
    noStroke();  //kein Rahmen der folgenden Formen
    rect(790,205,170,40);  //zeichne Rechteck
    
    if(mouseX>=790&&mouseX<=960&&mouseY>=255&&mouseY<=295)  //Wenn Mauscursor über Schaltfläche für Dämmerungsschalter, dann...
    {
      fill(150);  //setze die Füllfarbe auf Greyscale Wert 150
    }
    else  //Sonst...
    ...
    if(daemmerung||((mousePressed==true&&mouseButton==LEFT)&&(mouseX>=790&&mouseX<=960&&mouseY>=255&&mouseY<=295)))  //Wenn toggle für Dämmerungsschalter=true oder in die Schaltfläche für Dämmerungsschalter geklickt wird, dann...
    {
      zeitschaltung=false;  //setze toggle für Zeitschaltung=false und für Dämmerungsschalter=true
      daemmerung=true;      
      fill(23,94,209);  //setze die Füllfarbe auf RGB Wert (23,94,209)
    }
    noStroke();  //kein Rahmen der folgenden Formen
    rect(790,255,170,40);  //zeichne Rechteck
    ...
    fill(0);  //setze die Schriftfarbe auf Greyscale Wert 0
    textAlign(CENTER,CENTER);  //Textausrichtung
    text("Zeitschaltung",875,223);  //schreibe "Fenster auf/zu" auf das Canvas
    text("Dämmerung",875,273);  //schreibe "Fenster auf/zu" auf das Canvas
    
    
    textSize(40);  //setze die Schriftgröße auf 40pt
    fill(0);  //setze die Schriftfarbe auf Greyscale Wert 0
    textAlign(LEFT,TOP);  //Textausrichtung
    text("Temperatur: "+temperatur,50,25);  //schreibe die Sensorbezeichnungen und Sensorwerte auf das Canvas
    text("Luftfeuchtigkeit: "+luftfeuchtigkeit,50,125);
    text("Bodenfeuchtigkeit: "+bodenfeuchtigkeit,50,225);
    text("Helligkeit: "+helligkeit,50,325);
  ...
  else  //keine Anzeige der GUI -> Screensaver
  {
    ...
    fill(0);  //setze die Schriftfarbe auf Greyscale Wert 0
    textAlign(CENTER,CENTER);  //Textausrichtung
    text(uhrzeitKomplettStr,385,300);  //schreibe die aktuelle Uhrzeit auf das Canvas
    textAlign(CENTER,CENTER);  //Textausrichtung
    text(datumKomplettStr,615,300);  //schreibe das aktuelle Datum auf das Canvas
    
    imageMode(CENTER);  //Bildausrichtung
    image(logo,500,500,250,250);  //setze das Logo auf das Canvas
    noFill();  //keine Füllung der folgenden Formen
    stroke(0);  //Strichfarbe Greyscale Wert 0
    strokeWeight(3);  //Strichstärke 3 Pixel
    rect(375,375,250,250);  //zeichne Rechteck
  }
...
void serialEvent(Serial Port)
{
  ...
  else  //Sonst (seriellerKontakt==true) = Kontakt bereits hergestellt
  {
    if(gui&&mousePressed==true&&mouseButton==LEFT)  //Wenn GUI angezeigt wird und linke Maustaste gedrückt, dann...
    {
      ...
      if(mouseX>=790&&mouseX<=960&&mouseY>=255&&mouseY<=295)  //Wenn Mauscursor über Schaltfläche für Dämmerung, dann...
      {
        datenSenden[0]=4;
      }
      
      Port.write(datenSenden[0]);  //schreibe zu sendende Daten in seriellen Port
    ...
    if(datenCount>9)  //Wenn Zählvariable größer 9 (zehn Einträge in Array), dann...
    {
      ...
      temperatur=datenEmpfangen[6];
      luftfeuchtigkeit=datenEmpfangen[7];
      bodenfeuchtigkeit=datenEmpfangen[8];
      helligkeit=datenEmpfangen[9];
    ...
    if(rechteBenutzer1==1||rechteBenutzer2==1)  //Wenn Benutzerrechte=master, dann... (Anzeige der Protokollierung nur für Benutzer mit master Rechten)
    {
